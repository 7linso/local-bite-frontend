...
const { recipe, getRecipe, errors, deleteOpen, deleteRecipe } = useRecipe(
  auth,
  toast
);
...
const editRecipe = () => {
  if (recipe.value?._id) router.push(`/recipes/${recipe.value._id}/edit`);
};
...
const handleLikeRecipe = async () => {
  ...
  try {
    if (prevLiked) await recipes.dislikeRecipe(recipe.value._id);
    else await recipes.likeRecipe(recipe.value._id);
