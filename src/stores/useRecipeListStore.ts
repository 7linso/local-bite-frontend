...
export const useRecipeListStore = defineStore("recipes", () => {
  ...
  const main = {
    ...
    geojson: ref<FeatureCollection<Point>>({
  ...
  const fetchFirstPage = async () => {
    ...
    try {
      ...
      main.error.value =
        e?.data?.message ?? e?.message ?? "Failed to load recipes";
      main.list.value = [];
  ...
  const fetchNextPage = async () => {
    ...
    try {
      const res = await recipes.getAllRecipes(
        buildParams({ cursor: main.nextCursor.value })
      );
      ...
      main.error.value =
        e?.data?.message ?? e?.message ?? "Failed to load more recipes";
    } finally {
      main.loading.value = false;
    }
  };
  ...
  const fetchPointRecipes = async (lat: number, lng: number) => {
    ...
    try {
      ...
      point.error.value =
        e?.data?.message ??
        e?.message ??
        "Failed to load recipes for this location";
      point.recipes.value = [];
      point.open.value = true;
    } finally {
  ...
  const fetchUsersRecipes = async () => {
    ...
    try {
      ...
      users.error.value =
        e?.data?.message ?? e?.message ?? "Failed to load user recipes";
      users.list.value = [];
      users.nextCursor.value = null;
      users.hasNextPage.value = false;
    } finally {
  ...
  const fetchNextUsersRecipes = async () => {
    ...
    try {
      ...
      users.error.value =
        e?.data?.message ?? e?.message ?? "Failed to load more liked recipes";
    } finally {
      users.loading.value = false;
    }
  };
  ...
  const fetchUsersLikedRecipes = async () => {
    ...
    try {
      ...
      liked.error.value =
        e?.data?.message ?? e?.message ?? "Failed to load liked recipes";
      liked.list.value = [];
      liked.nextCursor.value = null;
      liked.hasNextPage.value = false;
    } finally {
  ...
  const fetchNextUsersLikedRecipes = async () => {
    ...
    liked.error.value = null;
    try {
      const res = await recipes.getLikedRecipes({
        limit: 20,
        cursor: liked.nextCursor.value,
      });
  ...
  const fetchAuthorFirstPage = async (authorId: string) => {
  ...
  const fetchAuthorNextPage = async (authorId: string) => {
    ensureAuthor(authorId);
    const s = authors[authorId];
    if (!s) return;
